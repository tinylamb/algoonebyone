...
public final class StdIn {
    ...
    private static final Pattern ALL_PATTERN = Pattern.compile("\\Z");
   ...
   /**
    ...
    public static String readFileAll(File file) {
        ...
        //String result = scanner.useDelimiter(ALL_PATTERN).next();
        String result = scanner.useDelimiter(EVERYTHING_PATTERN).next();
        scanner.useDelimiter(WHITESPACE_PATTERN);
        scanner.close();
        return result;
   ...
   /**
    ...
    public static String[] readFileAllStrings(File file) {
        String[] tokens = WHITESPACE_PATTERN.split(readFileAll(file));
        if (tokens.length == 0 || tokens[0].length() > 0) {
            return tokens;
        }
